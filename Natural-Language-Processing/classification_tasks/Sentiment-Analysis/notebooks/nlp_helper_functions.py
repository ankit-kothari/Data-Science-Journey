...
def convert_lines_to_feature_strings_extensive(lines, stopwords, remove_stopword_bigrams=True):
    ...
    for line in tqdm(lines):
        ...
        exclude_list = ['mr', 'mr.', '\'s', 'n\'t',
                        'managers', 'ms.', 'a.m.', 'p.m.', 'ms', 'Ms']
...
emotions = pd.read_csv(
    '/workspaces/Data-Science-Journey/Natural-Language-Processing/Transformers/Sentiment Analysis/input/word_emotion_intnsity.csv')
...
def get_word2emotion_mapping():
    ...
    '''
    emotion = emotions[['English (en)', 'Anger', 'Anticipation',
...
def word2emotion_matrix(wte, lines, stopwords):
    ...
    for line in tqdm(lines):
        ...
        emotion_frame_normalized_count = emotion_frame_raw_count.div(
            emotion_frame_raw_count.sum(axis=1), axis=0)
...
def word2polarity_matrix(wtp, lines, stopwords):
    ...
    for line in tqdm(lines):
        ...
        polarity_frame_raw_count = pd.DataFrame(
            polarity_matrix, columns=['positive', 'negative'])
        polarity_frame_normalized_count = polarity_frame_raw_count.div(
            polarity_frame_raw_count.sum(axis=1), axis=0)
...
def get_verb_pairs(lines, dep_tag, filter=False):
    ...
    if filter:
        ...
        filtered_verbs_pairs = dict(
            sorted(filtered_verbs_pairs.items(), key=lambda x: x[1], reverse=True))

        for verb, verb_count in verb_counts.items():
            if verb not in stop_words:
                filtered_verbs[verb] = verb_count
        filtered_verbs = dict(
            sorted(filtered_verbs.items(), key=lambda x: x[1], reverse=True))
        return filtered_verbs, filtered_verbs_pairs

    verb_counts = dict(
        sorted(verb_counts.items(), key=lambda x: x[1], reverse=True))
    verb_objects = dict(sorted(verb_objects.items(),
